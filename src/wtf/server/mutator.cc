...
LibfuzzerMutator_t::Create(std::mt19937_64 &Rng, const size_t TestcaseMinSize, const size_t TestcaseMaxSize) {
  return std::make_unique<LibfuzzerMutator_t>(Rng, TestcaseMinSize, TestcaseMaxSize);
}

LibfuzzerMutator_t::LibfuzzerMutator_t(std::mt19937_64 &Rng,
                                       const size_t TestcaseMinSize,
                                       const size_t TestcaseMaxSize)
    : Rand_(Rng()), Mut_(Rand_, fuzzer::FuzzingOptions()),
      TestcaseMinSize_(TestcaseMinSize), TestcaseMaxSize_(TestcaseMaxSize) {
...
std::string LibfuzzerMutator_t::GetNewTestcase(const Corpus_t &Corpus) {
  ...
  if (Testcase->BufferSize_ < TestcaseMinSize_)
  {
    memset(ScratchBuffer_.data() + Testcase->BufferSize_, 0, TestcaseMinSize_ - Testcase->BufferSize_);
  }

  const size_t NewTestcaseSize =
      Mut_.Mutate(ScratchBuffer_.data(), std::max(TestcaseMinSize_, Testcase->BufferSize_), 
                  TestcaseMinSize_, ScratchBuffer_.size_bytes());
...
std::unique_ptr<Mutator_t>
HonggfuzzMutator_t::Create(std::mt19937_64 &Rng, const size_t TestcaseMinSize, const size_t TestcaseMaxSize) {
  return std::make_unique<HonggfuzzMutator_t>(Rng, TestcaseMinSize, TestcaseMaxSize);
}

HonggfuzzMutator_t::HonggfuzzMutator_t(std::mt19937_64 &Rng,
                                       const size_t TestcaseMinSize,
                                       const size_t TestcaseMaxSize)
    : Rng_(Rng), Run_(Rng_), TestcaseMinSize_(TestcaseMinSize), TestcaseMaxSize_(TestcaseMaxSize) {
...
std::string HonggfuzzMutator_t::GetNewTestcase(const Corpus_t &Corpus) {
  ...
  const size_t NewTestcaseSize =
      Mutate(ScratchBuffer_.data(), std::max(TestcaseMinSize_, Testcase->BufferSize_),
