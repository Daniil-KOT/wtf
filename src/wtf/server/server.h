...
#pragma once
#include "../globals/corpus.h"
#include "../globals/dirwatch.h"
...
using VecSetPair = std::pair<std::vector<Gva_t>, tsl::robin_set<Gva_t>>;
...
class Server_t {
  ...
  //
  //  Map that contains testcase hashes and its overall coverage
  //  that is used to create diffs later on.
  //

  std::map<std::string, VecSetPair> Coverages_;

  //
  //  Coverage counter.
  //

  //CoverageCounter CovCounter_;

public:
  explicit Server_t(const Options_t &Opts)
      : Opts_(Opts), Rng_(Opts.Master.Seed), Corpus_(Opts.Master.OutputsPath, Rng_) {
    FD_ZERO(&ReadSet_);
    FD_ZERO(&WriteSet_);
  }
  ...
  int Run(const Target_t &Target) {
    ...
    //
    //  Let's check if counter succeeded in counting overall number of instructions
    //

    // if (!CovCounter_.Success())
    // {
    //   fmt::print("Unable to calculate overall number of instructions.\n"
    //              "It means that you won't get coverage percent at the end of fuzzing.\n"
    //              "Sorry!\n");
    // }
    // else
    //   fmt::print("Total number of instructions calculated: {}\n", CovCounter_.OverallCov());

    //
    // Initialize our internal state.
    //

    ScratchBufferGrip_ = std::make_unique<uint8_t[]>(_1MB * 10);
    ScratchBuffer_ = {ScratchBufferGrip_.get(), _1MB * 10};

    if (Opts_.Master.TestcaseBufferMaxSize > ScratchBuffer_.size_bytes()) {
      fmt::print("The biggest testcase would not fit in the scratch buffer\n");
      return EXIT_FAILURE;
    }

    if (Opts_.Master.TestcaseBufferMinSize > Opts_.Master.TestcaseBufferMaxSize)
    {
      fmt::print("min_size ({}) cannot be bigger than max_size ({})", 
                 Opts_.Master.TestcaseBufferMinSize, Opts_.Master.TestcaseBufferMaxSize);
      return EXIT_FAILURE;
    }
    ...
    Mutator_ = Target.CreateMutator(Rng_, Opts_.Master.TestcaseBufferMinSize, Opts_.Master.TestcaseBufferMaxSize);
    ...
    const fs::directory_iterator DirIt(Opts_.Master.InputsPath);
    ...
    fmt::print("Running server on {}..\n", Opts_.Master.Address);
    auto ServerOpt = Listen(Opts_.Master.Address);
    if (!ServerOpt) {
    ...
    while (Ret == EXIT_SUCCESS) {
      ...
      if (Opts_.Master.Runs != 0 && Mutations_ >= Opts_.Master.Runs && Paths_.size() == 0) {
  ...
  template<typename T>
  std::pair<bool, std::string> CreateFileData(T FirstGvas, T SecondGvas, 
                                              std::string FirstHash, 
                                              std::string SecondHash)
  {
    bool Second = false;
    bool Equal = FirstGvas.size() == SecondGvas.size();
    bool Written = false;
    std::stringstream Data;

    if (!Equal && FirstGvas.size() < SecondGvas.size())
    {
      Second = true;
    }

    auto it1 = FirstGvas.begin();
    auto it2 = SecondGvas.begin();
    Data << "\t\t\t" << FirstHash << ":" << "\t\t" << SecondHash << ":\n"; 
    uint32_t Line = 1;
    for (; it1 != FirstGvas.end() && it2 != SecondGvas.end(); ++it1, ++it2)
    {
      if (it1->U64() != it2->U64())
      {
        std::string FirstGva = fmt::format("{:#x}", it1->U64());
        std::string SecondGva = fmt::format("{:#x}", it2->U64());
        Data << "Line " << Line << ": \t" << FirstGva <<
                std::string((FirstHash.size() + 8) - FirstGva.size(), ' ') << 
                SecondGva << "\n";
        Written = true;
      }
      ++Line;
    ...
    if (Second)
    {
      ...
      for(; it2 != SecondGvas.end(); ++it2)
      {
        Data << "Line " << Line << ": \t" << std::string((FirstHash.size() + 8), ' ') << 
                fmt::format("{:#x}", it2->U64()) << "\n";
        ++Line;
      }
    }
    else if (!Equal)
    {
      ...
      for(; it1 != FirstGvas.end(); ++it1)
      {
        Data << "Line " << Line << ": " << "\t" << fmt::format("{:#x}", it1->U64()) << "\n";
        ++Line;
      }
    }

    return std::make_pair(Written, Data.str());
  }

  //
  //  Creates and saves coverage diffs.
  //

#define Hash      first
#define Vector    second.first
#define RobinSet  second.second

  bool MakeCoverageDiffs()
  {
    fmt::print("Creating diff files.\n"
               "Please wait a little bit...\n");

    std::ofstream DiffOutput;

    for(auto it1 = Coverages_.begin(); it1 != Coverages_.end();)
    {
      for (auto it2 = ++it1; it2 != Coverages_.end(); ++it2)
      {
        std::string Filename = it1->Hash + "-" + it2->Hash;
        fs::path OutputPath(Opts_.Master.DiffsPath / Filename);
        ...
        if (Opts_.FullTrace)
        {
          auto &&[Success, Str] = CreateFileData(it1->Vector, it2->Vector,
                                                 it1->Hash, it2->Hash);
          Written = Success;
          Data = Str;
        }
        ...
        {
          auto &&[Success, Str] = CreateFileData(it1->RobinSet, it2->RobinSet,
                                                 it1->Hash, it2->Hash);
          Written = Success;
          Data = Str;
        }
        ...
        if (Written)
        {
          DiffOutput.open(OutputPath, std::ios::binary);

          if (!DiffOutput.is_open())
          {
            fmt::print("Failed to create file.\n");
            return false;
          }

          DiffOutput.write(Data.data(), Data.size());
          DiffOutput.close();
        }
      }
    }
...
private:
  ...
  std::string GetTestcase(const Target_t &Target) {
    ...
    if (Paths_.size() > 0) {
      ...
      while (!FoundFile && Paths_.size() > 0) {
        ...
        const bool Valid =
            BufferSize > 0 && BufferSize <= Opts_.Master.TestcaseBufferMaxSize;

        if (!Valid) {
          fmt::print("Skipping because {} size is zero or bigger than the max "
                     "({} vs {})\n",
                     Path.string(), BufferSize, Opts_.Master.TestcaseBufferMaxSize);
        }
  ...
  //
  // Try to emplace address in a set
  //

  template<typename T>
  void TryEmplaceNewCov(T Coverage)
  {
  ...
  bool HandleNewResult(const SocketFd_t Fd, const Target_t &Target) {
    ...
    if (Opts_.FullTrace)
    {
      Deserialized = DeserializeResult(ReceivedBuffer, 
                                       ReceivedTestcase, 
                                       CoverageV,
                                       Result);
    }
    ...
    if (Opts_.FullTrace ? (CoverageV.size() > 0) : (Coverage.size() > 0)) {

      bool NewCovHash = false;

      if (Opts_.FullTrace)
      {
        std::string Gvas = "";
        for (auto &Gva : CoverageV)
        {
          Gvas += std::to_string(Gva.U64());
        }

        std::string CoverageHash = Blake3HexDigest((uint8_t*)Gvas.data(), Gvas.size());
        const auto &[_, Res] = Hashes_.emplace(CoverageHash);
        NewCovHash = Res;
      }
      ...
      if (Opts_.FullTrace)
        TryEmplaceNewCov(CoverageV);
      else
        TryEmplaceNewCov(Coverage);
      ...
      if (NewCoverage || NewCovHash) {
        ...
        auto &&[ShouldSave, TestcaseHash] = Corpus_.SaveTestcase(Result, std::move(Testcase));

        //
        //  Saving testcase hash and coverage to use it for diffs later on.
        //

        if (ShouldSave)
          Coverages_.emplace(TestcaseHash, VecSetPair(CoverageV, Coverage));
      }
    }
    ...
    if (const auto &Crash = std::get_if<Crash_t>(&Result)) {
      if (Crash->CrashName.size() > 0) {
        const auto &OutputPath = Opts_.Master.CrashesPath / Crash->CrashName;
        const auto &Success =
