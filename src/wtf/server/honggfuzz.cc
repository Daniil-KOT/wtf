...
static inline size_t mangle_getLen(run_t *run, size_t max) {
  ...
  const uint64_t rnd = util_rndGet(run, 1, max2 - 1);
...
static inline size_t mangle_getOffSet(run_t *run) {
  return mangle_getLen(run, run->dynfile->size) - 1;
}
...
static inline size_t mangle_getOffSetPlus1(run_t *run) {
  ...
  return mangle_getLen(run, reqlen) - 1;
}
...
static void mangle_MemSwap(run_t *run, bool) {
  ...
  size_t len = mangle_getLen(run, HF_MIN(maxlen1, maxlen2));
...
static void mangle_MemCopy(run_t *run, bool printable HF_ATTR_UNUSED) {
  ...
  size_t len = mangle_getLen(run, run->dynfile->size - off);
...
static void mangle_Bytes(run_t *run, bool printable) {
  uint16_t buf;
  if (printable) {
    util_rndBufPrintable(run, (uint8_t *)&buf, sizeof(buf));
  } else {
    buf = util_rnd64(run);
  }

  /* Overwrite with random 1-2-byte values */
  size_t toCopy = util_rndGet(run, 1, 2);
...
static void mangle_ByteRepeatOverwrite(run_t *run, bool printable) {
  ...
  size_t len = mangle_getLen(run, maxSz);
...
static void mangle_Bit(run_t *run, bool printable) {
  size_t off = mangle_getOffSet(run);
  run->dynfile->data[off] ^= (uint8_t)(1U << util_rndGet(run, 0, 7));
  if (printable) {
...
static void mangle_Magic(run_t *run, bool printable) {
  uint64_t choice = util_rndGet(run, 0, ARRAYSIZE(mangleMagicVals) - 1);
...
static void mangle_StaticDict(run_t *run, bool printable) {
  ...
  uint64_t choice = util_rndGet(run, 0, run->global->mutate.dictionaryCnt - 1);
...
static inline const uint8_t *mangle_FeedbackDict(run_t *run, size_t *len) {
  ...
  uint32_t choice = util_rndGet(run, 0, cnt - 1);
...
static void mangle_MemSet(run_t *run, bool printable) {
  size_t off = mangle_getOffSet(run);
  size_t len = mangle_getLen(run, run->dynfile->size - off);
  int val = printable ? (int)util_rndPrintable(run)
                      : (int)util_rndGet(run, 0, UINT8_MAX);
...
static void mangle_RandomOverwrite(run_t *run, bool printable) {
  ...
  size_t len = mangle_getLen(run, run->dynfile->size - off);
  if (printable) {
    util_rndBufPrintable(run, &run->dynfile->data[off], len);
  } else {
    util_rndBuf(run, &run->dynfile->data[off], len);
  }
}
...
static inline void mangle_AddSubWithRange(run_t *run, size_t off, size_t varLen,
  ...
  int64_t delta = (int64_t)util_rndGet(run, 0, range * 2) - (int64_t)range;
...
static void mangle_AddSub(run_t *run, bool printable) {
  ...
  /* 1,2,4,8 */
  size_t varLen = 1U << util_rndGet(run, 0, 3);
...
static void mangle_Expand(run_t *run, bool printable) {
  size_t off = mangle_getOffSet(run);
  size_t len;
  if (util_rnd64(run) % 16) {
    len = mangle_getLen(run, HF_MIN(16, run->global->mutate.maxInputSz - off));
  } else {
    len = mangle_getLen(run, run->global->mutate.maxInputSz - off);
  }
...
static void mangle_Shrink(run_t *run, bool printable HF_ATTR_UNUSED) {
  ...
  if (util_rnd64(run) % 16) {
    len = mangle_getLen(run, HF_MIN(16, len));
  } else {
    len = mangle_getLen(run, len);
  }
...
static void mangle_ASCIINum(run_t *run, bool printable) {
  size_t len = util_rndGet(run, 2, 8);

  char buf[20];
  snprintf(buf, sizeof(buf), "%-19" PRId64, (int64_t)util_rnd64(run));
...
static void mangle_ASCIINumChange(run_t *run, bool printable) {
  ...
  switch (util_rndGet(run, 0, 5)) {
  case 0:
    val += util_rndGet(run, 1, 256);
    break;
  case 1:
    val -= util_rndGet(run, 1, 256);
    break;
...
static void mangle_Splice(run_t *run, bool printable) {
  ...
  size_t remoteOff = mangle_getLen(run, sz) - 1;
  size_t len = mangle_getLen(run, sz - remoteOff);
...
static void mangle_Resize(run_t *run, bool printable) {
  ...
  uint64_t choice = util_rndGet(run, 0, 32);
  switch (choice) {
  case 0: /* Set new size arbitrarily */
    newsz = (ssize_t)util_rndGet(run, 1, run->global->mutate.maxInputSz);
    break;
  case 1:
  case 2:
  case 3:
  case 4: /* Increase size by a small value */
    newsz = oldsz + (ssize_t)util_rndGet(run, 0, 8);
    break;
  case 5: /* Increase size by a larger value */
    newsz = oldsz + (ssize_t)util_rndGet(run, 9, 128);
    break;
  case 6:
  case 7:
  case 8:
  case 9: /* Decrease size by a small value */
    newsz = oldsz - (ssize_t)util_rndGet(run, 0, 8);
    break;
...
void mangle_mangleContent(run_t *run, int speed_factor) {
  ...
  if (speed_factor < 5) {
    changesCnt = util_rndGet(run, 1, run->global->mutate.mutationsPerRun);
  ...
  for (uint64_t x = 0; x < changesCnt; x++) {
    uint64_t choice = util_rndGet(run, 0, ARRAYSIZE(mangleFuncs) - 1);
