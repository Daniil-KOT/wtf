  ...
  if (!g_Backend->SetBreakpoint(
          "ntdll!RtlDispatchException", [](Backend_t *Backend) {
            ...
            fmt::print("Address: {:#x}\n"
                       ...
                       ExceptionCodeToStr(ExceptionRecord.ExceptionCode),
                       ExceptionRecord.ExceptionFlags);

            fmt::print("Info: {} @ {:#x}\n", ExceptionRecord.ExceptionInformation[0], ExceptionRecord.ExceptionInformation[0]);
            for (int i = 1; i < EXCEPTION_MAXIMUM_PARAMETERS; ++i)
            {
              fmt::print("\t  {} @ {:#x}\n", ExceptionRecord.ExceptionInformation[i], ExceptionRecord.ExceptionInformation[i]);
            }
            fmt::print("Num params: {}\n", ExceptionRecord.NumberParameters);
