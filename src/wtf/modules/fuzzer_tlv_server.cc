...
bool Init(const Options_t &Opts, const CpuState_t &State) {
  ...
  // Example tlv_server!ProcessPacket address -- 0x7ff719e51150
  // for some reason tlv_server!ProcessPacket symbol is not processed correctly
  // so I decided to replace symbol with an address
  if (!g_Backend->SetBreakpoint(
          Gva_t(0x7ff719e51150), [](Backend_t *Backend) {
  ...
  // Example tlv_server!printf address -- 0x7ff719e510f0
  // same as with tlv_server!ProcessPacket
  if (!g_Backend->SetBreakpoint(Gva_t(0x7ff719e510f0), [](Backend_t *Backend) {
