...
#include "../backend/bochscpu_backend.h"
#include "../backend/kvm_backend.h"
#include "../globals/platform.h"
#include "../base/subcommands.h"
#include "../globals/utils.h"
#include "../backend/whv_backend.h"
#include "../handles/irqhandler.h"
#include <filesystem>
...
int main(int argc, const char *argv[]) {
  ...
  CLI::App *MasterCmd =
      Wtf.add_subcommand("master", "Master options")->callback([&Opts] {
        ...
        if (Opts.Master.DiffsPath.empty()) {
          Opts.Master.DiffsPath = Opts.Master.TargetPath / "diffs";
        }
        ...
        if (!std::filesystem::exists(Opts.Master.InputsPath) ||
            ...
            !std::filesystem::exists(Opts.Master.DiffsPath)) {
          fmt::print("Expected to find an inputs directory, an outputs "
                     "directory, a crashes directory and a diffs directory in {}.\n",
  ...
  MasterCmd->add_option("--binary_name", Opts.Master.BinaryName, "Binary name")
      ->description("Name of the binary with an extension (must be present in harness/ directory).");

  MasterCmd->add_option("--full_trace", Opts.FullTrace, "Full trace")
      ->description("Tell fuzzer to generate full traces (default is false).");
  ...
  MasterCmd
      ->add_option("--min_len", Opts.Master.TestcaseBufferMinSize,
                   "Testcase size")
      ->description("Minimum size of a generated testcase.")
      ->required();
  ...
  MasterCmd->add_option("--diffs", Opts.Master.DiffsPath, "Diff files")
      ->description("Diffs path");

  MasterCmd->add_option("--create_diffs", Opts.Master.CreateDiffs, "Create diffs.")
      ->description("Tell fuzzer to create diff files (default is false).");
  ...
  RunCmd->add_option("--full_trace", Opts.FullTrace, "Binary name")
      ->description("Tell fuzzer to generate full traces (default is false).");
  ...
  FuzzCmd
      ->add_option("--full_trace", Opts.FullTrace, "Binary name")
      ->description("Tell fuzzer to generate full traces (default is false).");
  ...
  if (Wtf.got_subcommand("master")) {
    Server_t Srv(Opts);
    IrqHandler::SetInstance(&Srv);
    if (!SetConsoleCtrlHandler(IrqHandler::InterruptHandler, TRUE))
    {
      fmt::print("Could not set Ctrl Handler!\n");
    }
    return MasterSubcommand(Srv, *Target);
  }
