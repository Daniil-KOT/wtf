...
TestcaseResult_t RunTestcaseAndRestore(const Target_t &Target,
  ...
  if (FullTrace)
    g_Stats.TestcaseEnds(*Res, g_Backend->LastNewCoverageV().size());
  else
    g_Stats.TestcaseEnds(*Res, g_Backend->LastNewCoverage().size());
...
int Client_t::Run(const Target_t &Target, const CpuState_t &CpuState) {
  ...
  while (1) {
    ...
    const TestcaseResult_t TestcaseResult = RunTestcaseAndRestore(
        Target, CpuState, {(uint8_t *)Testcase.data(), Testcase.size()}, false, Opts_.FullTrace);
    ...
    bool Success;
    if (Opts_.FullTrace)
      Success = SendResult(Client_, Testcase, g_Backend->LastNewCoverageV(), TestcaseResult);
    else
      Success = SendResult(Client_, Testcase, g_Backend->LastNewCoverage(), TestcaseResult);
