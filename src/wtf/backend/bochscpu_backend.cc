...
bool BochscpuBackend_t::Initialize(const Options_t &Opts,
  ...
  //
  //  Tell if we have to generate full trace
  //

  FullTrace_ = Opts.FullTrace;
...
__declspec(safebuffers)
  ...
  if (Res.second && !FullTrace_) 
  {
    LastNewCoverage_.emplace(Rip);
  }
  else if (FullTrace_)
  {
    LastNewCoverageV_.push_back(Rip);
  }
...
void BochscpuBackend_t::TlbControlHook(/*void *Context, */ uint32_t,
  ...
  fmt::print("The cr3 register is getting changed ({:#x}) --> ({:#x})\n", 
              InitialCr3_, NewCrValue);
...
const std::vector<Gva_t> &BochscpuBackend_t::LastNewCoverageV() const {
  return LastNewCoverageV_;
}
